...
- **Anthropic / OpenAI-style keys** — persisted via the Svelte `settings` store to **`localStorage`** under `tinyllama.settings.v3` when running in the app shell (Tauri webview). This keeps keys out of the repo but is **not** OS-level secret storage; treat the profile directory accordingly on shared machines.
- **Planned:** migrate API keys to OS keychain (Stronghold) and proxy LLM HTTP from Rust so keys are not available to the webview.
...
If a key was pasted into Settings during testing and the machine is shared or compromised: revoke/regenerate it at the provider, clear site data for the app webview, or remove `tinyllama.settings.v3` from local storage after uninstalling.

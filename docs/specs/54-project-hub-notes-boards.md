# Spec 54 — Universal Project Hub, Notes & Boards
...
> **Depends on:** [53-system-tray-desktop-assistant.md](53-system-tray-desktop-assistant.md) for tray/global-hotkey plumbing only — **does not** require any of Spec 53's `system_*` write tools or its trust escalation model. This spec is lower-risk (local index + launching, no OS mutation) and can ship independently of, and before, Spec 53's later phases.
...
- **Not in v1:** arbitrary user-scriptable "extensions" (Raycast's plugin model) or OS-level system actions — those belong to Spec 53's `system_*` tier and its trust model, not here.
...
| Mode | Trigger | Look |
|------|---------|------|
| **Palette** | Global hotkey (reuses Spec 53 §3's `tauri-plugin-global-shortcut`), auto-hides on blur/`Esc` | Small, centered/anchored, just the search input + ranked list |
...
- Global hotkey for the palette (via [37-shortcut-rebinding.md](37-shortcut-rebinding.md), shared with Spec 53's summon binding if both ship).
